...
proto.rangevalue = function(min, max, options, cb) {
  if(options.a === "exclusive"){
     min = "("+min
  }
  if(options.b === "exclusive"){
    max = "("+max
  }
  this.rangeValueIn(this.name, min, max, cb);
};

proto.rangeValueIn = function(name, min, max, cb) {
  this.redis.zcount([name, min, max], function(err, score) {
    if (err) return cb(err);
    if (score === null)
      cb(null, -1);
    else
...
proto.rangeByScoreInfo = function(min, max, options, cb) {
  if(options.a === "exclusive"){
     min = "("+min
  }
  if(options.b === "exclusive"){
  ...
  this.rangeByScoreInfoIn(this.name, min, max, cb);

};

proto.rangeByScoreInfoIn = function(name, min, max, cb) {
  console.log('reverse '+this.reverse)
  if(this.reverse){
    this.redis.zrangebyscore([name, min, max], response);
  }else{
    this.redis.ZREVRANGEBYSCORE([name, min, max], response);
  }


  function response(err,score){
    if (err) return cb(err);
    if (score === null)
      cb(null, -1);
    else
...
proto.removeRangeByRank = function(min, max, cb) {
  this.removeRangeByRankIn(this.name, min, max, cb);
};

proto.removeRangeByRankIn = function(name, min, max, cb) {
  this.redis.ZREMRANGEBYRANK([name, min, max], function(err, score) {
    if (err) return cb(err);
    if (score === null)
      cb(null, -1);
    else
...
proto.removeRangeByScore = function(min, max, options, cb) {
  if(options.a === "exclusive"){
     min = "("+min
  }
  if(options.b === "exclusive"){
  ...
  this.removeRangeByScoreIn(this.name, min, max, cb);

};

proto.removeRangeByScoreIn = function(name, min, max, cb) {
  this.redis.ZREMRANGEBYSCORE([name, min, max], function(err, score) {
    if (err) return cb(err);
    if (score === null)
      cb(null, -1);
    else
...
proto.getScoreByPattern = function(query, cb){
  this.getScoreByPatternIn(this.name, query, cb)
}

proto.getScoreByPatternIn = function(name, query, cb){
  this.redis.zscan([name, 0, 'match','*'+query+'*','count','100000'], function(err, score) {
    if (err) return cb(err);
    if (score === null)
      cb(null, -1);
    else
      var list = [], l = score[1].length;
      ...
      //console.log('score '+JSON.stringify(list))
      cb(null, list);
  });
}
